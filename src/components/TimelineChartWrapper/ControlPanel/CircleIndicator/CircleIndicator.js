...
export default class CircleIndicator extends Component {
  static L = ({ startColor, stopColor, x1, x2, y1, y2, id }) => {
  ...
  componentDidMount () {
    ...
    setTimeout(() => this.updateChart(this.props.percent))
  }

  componentWillReceiveProps ({ percent }) {
    if (this.props.percent !== percent) {
      this.updateChart(percent)
    }
  }

  updateChart (percent) {
    const td = selection => selection.transition().duration(CircleIndicator.ANIMATION_DURATION)
  ...
  render () {
    const { L, WIDTH: strokeWidth, RADIUS } = CircleIndicator
    const r = RADIUS - strokeWidth / 2

    return <div className={ styles['circle-indicator'] }>
      <svg viewBox="-10 -10 220 220" ref={ svg => this.svg = d3.select(svg) }>
        <defs>
          <L id="redyel" startColor="#35ABEC" stopColor="#356ADE" x1="0" y1="0" x2="1" y2="1" />
          <L id="yelgre" startColor="#356ADE" stopColor="#3547D7" x1="0" y1="0" x2="0" y2="1" />
          <L id="grecya" startColor="#3547D7" stopColor="#713DBD" x1="1" y1="0" x2="0" y2="1" />
          <L id="cyablu" startColor="#713DBD" stopColor="#D91E5D" x1="1" y1="1" x2="0" y2="0" />
          <L id="blumag" startColor="#D91E5D" stopColor="#EB0021" x1="0" y1="1" x2="0" y2="0" />
          <L id="magred" startColor="#EB0021" stopColor="#EB001E" x1="0" y1="1" x2="1" y2="0" />
        </defs>
